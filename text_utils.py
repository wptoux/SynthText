...
class RenderFont(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        # self.min_font_h = 32  # px : 0.6*12 ~ 7px <= actual minimum height
        ...
        self.text_source = TextSource(self.min_nchar, osp.join(data_dir, 'text'))
...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        self.font_model = pickle.load(open(font_model_path, 'rb'))
    ...
    def get_aspect_ratio(self, font, size=None):
        ...
        sizes = font.get_metrics('一', size)
        sizes = np.array(sizes).astype('float')[:, [3, 4]]
        return np.abs(sizes[:, 1] / sizes[:, 0])
...
class TextSource(object):
    ...
    def __init__(self, min_nchar, text_path):
        """
    ...
    def get_lines(self, nline, nword, nchar_max, f=0.35, niter=100):
        line_start = np.random.choice(len(self.txt))
        s = '.'.join(self.txt[line_start: line_start + 10])

        lines = []
        p = 0
        # approximate nchar = nword * 3
        for i in range(nline):
            line = s[p: p + nword[i] * 5]
            stop = min(len(line) - line.rfind(' '), nchar_max)
    ...
    #
    #             if np.any([is_chinese(ch) for ch in lines[i][:5]]):
